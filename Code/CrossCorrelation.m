function [Cor_Array,Timeaxis] = CrossCorrelation(Data1,Data2,Maxtime,Weights1,Weights2,mode,linear)
...
Timeaxis(Timeaxis > MaxMaxtime/2) = [];
if linear
    Timeaxis = (1:MaxMaxtime/2)';
end
...
for i=1:numel(Data1)
    if ~isempty(Data1{i}) && ~isempty(Data2{i})
        ...
        if ~linear
            ...
            Cor_Array{i} = (-1)*ones(1,numel(Timeaxis));
            c = xcorr(Weights1{i},Weights2{i});
            c = c(ceil(numel(c)/2)+1:end) + flipud(c(1:floor(numel(c)/2))-1);
            c = c/2 - 1; c(c==0) = -1;
            Cor_Array{i}(1:numel(c)) = c;
        end
