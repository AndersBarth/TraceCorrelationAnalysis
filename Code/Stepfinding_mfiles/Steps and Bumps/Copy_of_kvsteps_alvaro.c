...
)
{
    ...
    pf = mxGetScalar(PF);
    size = mxGetM(DATA_IN);  /* get it's size */
    ...
    SIC_opt=SIC(base->var,base->size,2,pf);//initialize optimum SIC
    ...
    for(;;) //empty loop control.  We'll terminate with a "break" below
    {
        ...
        while(currnode!=NULL) //break loop if there's no such thing as a next node
        {
            ...
            for(c=0;c<(currnode->size-1);c++)//test all possible change points in the current node
            {
                ...
                SIC_curr=SIC(grand_var,size,3+numsteps,pf);
                if(SIC_curr<SIC_opt)
                {
                    SIC_opt=SIC_curr;
                    steploc=c;
 ...
 *
 */
double SIC(double variance,int size,int params, int dummy)
{
    return pf*(double)params*log(size)+(double)size*log(variance);
}
