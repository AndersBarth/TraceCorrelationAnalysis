function [w_res,C] = FCS_two_state_kinetics_FRETFCS_threeexp(fit_parameters,t,cor,sem,method)
    ...
    A3_12= fit_parameters(12);
    if numel(fit_parameters) > 12 % offset
        offset = fit_parameters(13:end);
    else
        offset = zeros(1,4);
    end
    % compute ideal FCS curves
    C = zeros(numel(t),4);
    C(:,1) = A1_11*exp(-t./tau1)+A2_11*exp(-t./tau2)+A3_11*exp(-t./tau3) + offset(1); %DxD
    C(:,2) = A1_22*exp(-t./tau1)+A2_22*exp(-t./tau2)+A3_22*exp(-t./tau3)+ offset(2); %AxA
    C(:,3) = -(A1_12*exp(-t./tau1)+A2_12*exp(-t./tau2)+A3_12*exp(-t./tau3))+ offset(3); %DxA
    C(:,4) = -(A1_12*exp(-t./tau1)+A2_12*exp(-t./tau2)+A3_12*exp(-t./tau3))+ offset(4); %AxD
